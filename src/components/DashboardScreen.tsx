...
const DashboardScreen: React.FC = () => {
  const { trainee, loading, error } = useAppData();
    ...
    return (
      <div className="flex items-center justify-center h-full bg-zinc-50">
        <div className="text-center">
          <p className="text-zinc-500 font-bold">{t('processing_reschedule') || 'Loading...'}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center h-full bg-zinc-50">
        <div className="text-center p-6">
          ...
          <p className="text-zinc-500 text-sm">{error?.message || 'No trainee data available'}</p>
        </div>
      </div>
    );
  }
